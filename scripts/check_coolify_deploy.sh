...
DEPLOYMENT_UUID="${1:?deployment uuid required}"
...
SQL="INSERT INTO personal_access_tokens (tokenable_type, tokenable_id, name, token, abilities, team_id, created_at, updated_at) VALUES ('App\\\\Models\\\\User', 0, 'codex-status', '${TOKEN_HASH}', '[\"*\"]', 0, now(), now()) RETURNING id;"
